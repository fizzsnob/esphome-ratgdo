...
EntityMatchResult UrlMatch::match_entity(EntityBase *entity) const {
  ...
  // LOCAL COMPAT PATCH (ratgdo personal build): re-add the pre-2026.7
  // object-id fallback that upstream deleted in 2026.7.0. External REST
  // clients in the field (e.g. the RTI ratgdo driver) address entities by
  // object id (/cover/door); without this fallback every such GET and
  // command POST 404s. Matches the 2026.6.x behavior, minus the per-hit
  // deprecation warning (a 3s poll cycle would spam it); logs once.
  {
    char object_id_buf[OBJECT_ID_MAX_LEN];
    StringRef object_id = entity->get_object_id_to(object_id_buf);
    if (this->id == object_id) {
      result.matched = true;
      static bool objid_compat_logged = false;
      if (!objid_compat_logged) {
        objid_compat_logged = true;
        ESP_LOGD(TAG, "objid-compat: serving legacy object-id URL (/%s)", object_id_buf);
      }
    }
  }
