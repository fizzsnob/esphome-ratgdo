...
int RatgdoUART::available()
{
    ...
    if (length >= UART_RX_BUFFER_SIZE * 3 / 4) {
        // Warn while nearing capacity. The ring never reports a full 512, so the
        // old "== buffer size" check never fired. Diagnostic only — no flush, to
        // avoid dropping a valid in-flight frame.
